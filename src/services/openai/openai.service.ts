...
La valorisation du bien est calculée exclusivement par Apprexia Engine.

GPT ne doit jamais calculer, modifier ou remplacer :
...
Ces valeurs sont fournies par les moteurs Apprexia.

GPT doit uniquement les interpréter et expliquer leur cohérence avec :
- les prestations réellement détectées ;
- le prix affiché ;
- les données DVF ;
- les caractéristiques du bien.

Si une valeur calculée par Apprexia est fournie, utilise exactement cette valeur.

Ne recalcule jamais ces montants.

Si aucune donnée DVF fiable n'est disponible, les valeurs correspondantes restent null.
        ──────────────────────────────
    ...
    * des éléments présents dans l'annonce pouvant justifier une négociation.
    ...
    private mergePropertyFeatures(original?: PropertyFeatures, verified?: Partial<PropertyFeatures>): PropertyFeatures {
        return {
            duplex: verified?.duplex ?? original?.duplex ?? false,
            triplex: verified?.triplex ?? original?.triplex ?? false,
            loft: verified?.loft ?? original?.loft ?? false,

            terrasse: verified?.terrasse ?? original?.terrasse ?? false,
            balcon: verified?.balcon ?? original?.balcon ?? false,
            loggia: verified?.loggia ?? original?.loggia ?? false,
            jardin: verified?.jardin ?? original?.jardin ?? false,
            patio: verified?.patio ?? original?.patio ?? false,

            piscine: verified?.piscine ?? original?.piscine ?? false,
            jacuzzi: verified?.jacuzzi ?? original?.jacuzzi ?? false,
            spa: verified?.spa ?? original?.spa ?? false,
            sauna: verified?.sauna ?? original?.sauna ?? false,

            parking: verified?.parking ?? original?.parking ?? false,
            garage: verified?.garage ?? original?.garage ?? false,
            box: verified?.box ?? original?.box ?? false,

            cave: verified?.cave ?? original?.cave ?? false,
            grenier: verified?.grenier ?? original?.grenier ?? false,

            ascenseur: verified?.ascenseur ?? original?.ascenseur ?? false,
            gardien: verified?.gardien ?? original?.gardien ?? false,
            interphone: verified?.interphone ?? original?.interphone ?? false,
            digicode: verified?.digicode ?? original?.digicode ?? false,
            visiophone: verified?.visiophone ?? original?.visiophone ?? false,

            climatisation: verified?.climatisation ?? original?.climatisation ?? false,
            cheminee: verified?.cheminee ?? original?.cheminee ?? false,
            cuisineEquipee: verified?.cuisineEquipee ?? original?.cuisineEquipee ?? false,
            dressing: verified?.dressing ?? original?.dressing ?? false,
            buanderie: verified?.buanderie ?? original?.buanderie ?? false,

            vueMer: verified?.vueMer ?? original?.vueMer ?? false,
            vueMontagne: verified?.vueMontagne ?? original?.vueMontagne ?? false,
            vuePanoramique: verified?.vuePanoramique ?? original?.vuePanoramique ?? false,
            vueDegagee: verified?.vueDegagee ?? original?.vueDegagee ?? false,

            dernierEtage: verified?.dernierEtage ?? original?.dernierEtage ?? false,
            traversant: verified?.traversant ?? original?.traversant ?? false,
            lumineux: verified?.lumineux ?? original?.lumineux ?? false,
            calme: verified?.calme ?? original?.calme ?? false,

            renove: verified?.renove ?? original?.renove ?? false,
            standing: verified?.standing ?? original?.standing ?? false,
            prestige: verified?.prestige ?? original?.prestige ?? false,
        };
    }
