...
export class MetadataScraperService {
  ...
  async scrape(url: string): Promise<ListingMetadata> {
    ...
    try {
      ...
      this.logger.log(
        `Métadonnées insuffisantes, fallback Playwright: ${normalizedUrl}`,
      );
      ...
      this.logger.warn(
        `Erreur extraction HTML, fallback Playwright: ${normalizedUrl}`,
      );
  ...
  private async scrapeWithPlaywright(url: string): Promise<ListingMetadata> {
    ...
    try {
      ...
      if (title.includes('Just a moment') || title.includes('Cloudflare')) {
        console.log('⚠️ Cloudflare toujours actif');
  ...
  private async extractMetadata(
    ...
    const features = this.extractPropertyFeatures(title, description);
  ...
  private extractPropertyFeatures(
    title: string,
    description: string,
  ): PropertyFeatures {
    ...
    return {
      ...
      prestige: has('prestige', 'haut de gamme', 'exception', 'rare'),
    };
  }
  ...
  private async extractLocation(data: {
    ...
    if (!codePostal) {
      const cp = text.match(
        /\b(0[1-9]|[1-8]\d|9[0-5])\d{3}\b(?!\s*(€|euros?))/i,
      );
    ...
    // ----------------------------
    // 3. RESOLUTION COMMUNE VIA DVF + CP
    // ----------------------------

    if (codePostal) {
      const commune = await this.prisma.dvfTransaction.findFirst({
        where: {
          codePostal,
        },
        select: {
  ...
  private extractParisArrondissementCodePostal(
    text: string,
  ): string | undefined {
    if (!text) {
      return undefined;
  ...
  private normalizeCity(city?: string): string | undefined {
    ...
    return (
      city
        ...
        // suppression de tout caractère inutile
        .replace(/[^A-Za-z\s'-]/g, '')
  ...
  private extractStreetAddress(text: string): string | undefined {
    ...
    const regex = new RegExp(
      `\\b(?:${streetTypes})\\s+([A-Za-zÀ-ÿ0-9'’\\- ]{3,80})`,
      'i',
    );
